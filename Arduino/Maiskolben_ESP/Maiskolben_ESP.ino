...
//	digitalWrite(TFT_CS, HIGH);
  FastLED.addLeds<NEOPIXEL, HEAT_LED>(leds, NUM_LEDS);  // GRB ordering is assumed
  ...
  tft.setRotation(0);
		...
		EEPROM.write(EEPROM_DISPLAY, RGB_DISP);
		setDisplayMode(0);
	#else
		EEPROM.write(EEPROM_DISPLAY, BGR_DISP);
		setDisplayMode(1);
	#endif
#else
	...
	if (EEPROM.read(EEPROM_INSTALL) != EEPROM_CHECK) {
		...
		for (int16_t i = -127; i < 128; i++) {
			...
			//delay(1);
      leds[0] = CRGB::Red;
      FastLED.setBrightness( 127-abs(i) );
      FastLED.show();
      FastLED.delay(1);
		}
		...
		EEPROM.write(EEPROM_OPTIONS,  (fahrenheit << 2) | (bootheat << 1) | autopower);
		EEPROM.write(EEPROM_VERSION, EE_VERSION);
		EEPROM.write(EEPROM_INSTALL, EEPROM_CHECK);
	...
	tft.fillScreen(BLACK);
	uint8_t options = 0; //EEPROM.read(EEPROM_OPTIONS);
	autopower = options & 1;
	bootheat = options & 2;
  ...
  ITimer.attachInterruptInterval(TIMER_INTERVAL_MS * 1000, timer_isr);
...
void updateRevision(void) {	
	...
	EEPROM.write(EEPROM_REVISION, HARDWARE_REVISION);
	revision = 3;
#else
	if (EEPROM.read(EEPROM_VERSION) < 26 || EEPROM.read(EEPROM_REVISION) > 100) {
		EEPROM.write(EEPROM_REVISION, 2);
		revision = 2;
	} else {
...
void updateEEPROM(void) {
	EEPROM.write(1, stby);
	for (uint8_t i = 0; i < 3; i++) {
		EEPROM.write(2+i*2, stored[i] >> 8);
		EEPROM.write(3+i*2, stored[i] & 0xFF);
	}
	EEPROM.write(8, set_t >> 8);
	EEPROM.write(9, set_t & 0xFF);
	EEPROM.write(EEPROM_OPTIONS, (fahrenheit << 2) | (bootheat << 1) | autopower);
}
...
void display(void) {
	...
	if (cur_t_old != temperature || force_redraw) {
		...
		if (temperature < cur_t_old)
			tft.fillRect(max(0.0, ((double)temperature - TEMP_COLD)/2.4), 0, 160-max(0.0, ((double)temperature - TEMP_COLD)/2.4), BAR_HEIGHT, BLACK);
		else if (cur_t != 999) {
			for (int16_t i = max(0.0, ((double)cur_t_old - TEMP_COLD)/2.4); i < max(0.0, ((double)temperature - TEMP_COLD)/2.4); i++) {
				tft.drawFastVLine(i, 0, BAR_HEIGHT, tft.color565(min(255, max(0, i*5)), min(255.0, max(0.0, 450-i*2.5)), 0.0));
			}
		}
	...
	if (v_c3 > 1.0) {
		...
		tft.setCursor(122,5);
		tft.setTextSize(2);
		int power = min(15,(int)v)*min(15,(int)v)/4.8*pwm/255;
		...
		if (power_source == POWER_CORD) {
			/*if (v > v_c3) {
				...
				tft.drawBitmap(0, 5, power_cord, 24, 9, tft.color565(max(0, min(255, (int)((14.5-v)*112))), max(0, min(255, (int)((v-11)*112))), 0));
			//}
		} else if (power_source == POWER_LIPO || power_source == POWER_CHARGING) {
			float volt[] = {v_c1, v_c2-v_c1, v_c3-v_c2};
			uint8_t volt_disp[] = {max(1,min(16,(int)((volt[0]-3.0)*14.2))), max(1,min(16,(int)((volt[1]-3.0)*14.2))), max(1,min(16,(int)((volt[2]-3.0)*14.2)))};
			if (power_source == POWER_CHARGING) {
				uint8_t p = min(16, (int)((millis() / 100) % 20));
				for (uint8_t i = 0; i < 3; i++) {
					volt_disp[i] = max(0, min((int)(volt_disp[i]), (int)p));
				}
			}
			for (uint8_t i = 0; i < 3; i++) {
				if (volt[i] < 3.20) {
					...
					tft.fillRect(13, 7+14*i, volt_disp[i], 8, tft.color565(250-min(250.0, max(0.0, (volt[i]-3.4)*1000.0)), max(0.0,min(250.0, (volt[i]-3.15)*1000.0)), 0));
				}
...
uint16_t serialReadTemp() {
	...
	uint8_t n;
	n = Serial.read()-'0';
	t = min(9, max(0, (int)n))*100;
	n = Serial.read()-'0';
	t += min(9, max(0, (int)n))*10;
	n = Serial.read()-'0';
	t += min(9, max(0, (int)n))*1;
	return t;
}
